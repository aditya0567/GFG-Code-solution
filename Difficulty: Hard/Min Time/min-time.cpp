    ...
    long long minTime(int n, vector<int> &locations, vector<int> &types) {
        ...
        for(long long i = 0; i < types.size(); i++) {
            if (uni.count(types[i]) == 0) {
                ...
                if (pos[types[i]][1] < locations[i]) { // Changed 'else if' to 'if'
                    pos[types[i]][1] = (long long)locations[i];
                }
            }
        }
        ...
        long long i = 1LL;
        for (auto it : uni) { // Iterating over unique types
            long long ty = it;
            
            long long left = pos[ty][0];
            long long right = pos[ty][1];
            // Left to right
            long long nc = min(dp[i - 1][0] + abs(dp[i - 1][1] - left) + (right - left),
                               dp[i - 1][2] + abs(dp[i - 1][3] - left) + (right - left));
            dp[i][0] = nc;
            dp[i][1] = right;
            
            // Right to left
            nc = min(dp[i - 1][0] + abs(dp[i - 1][1] - right) + (right - left),
                     dp[i - 1][2] + abs(dp[i - 1][3] - right) + (right - left));
            dp[i][2] = nc;
            dp[i][3] = left;
            i++;
        }
